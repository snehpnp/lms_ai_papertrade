...
import { CreditCard, ShieldCheck, Save, Loader2, Activity, Wifi, WifiOff, Globe, ExternalLink, Info, Mail, Send, Lock, Server, Palette, Upload, ImagePlus, X, Trash2, Power } from "lucide-react";
...
const SettingsPage = () => {
    ...
    const [livePriceEnabled, setLivePriceEnabled] = useState(false);
    ...
    const fetchSettings = async () => {
        try {
            ...
            setLivePriceEnabled(data.find((s) => s.key === "LIVE_PRICE_ENABLED")?.value === "true");
    ...
    const handleSaveAliceBlue = () => saveSection("Alice Blue", [
        ...
        { key: "LIVE_PRICE_ENABLED", value: livePriceEnabled ? "true" : "false", description: "Enable/Disable Live Market Data streaming" },
    ]);
    ...
    return (
        <div className="animate-fade-in space-y-6">
            ...
            <Tabs defaultValue="integrations" className="w-full">
                <TabsList className="mb-6 bg-muted/50 border border-border/50 p-1 rounded-lg">
                    <TabsTrigger value="integrations" className="rounded-md">Integrations & API</TabsTrigger>
                    <TabsTrigger value="email" className="rounded-md">Email / Webmail</TabsTrigger>
                    <TabsTrigger value="branding" className="rounded-md">Appearance & Branding</TabsTrigger>
                </TabsList>
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                        ...
                        <Card>
                                ...
                                <div className="pt-4 border-t border-border/50">
                                    <div className="flex items-center justify-between p-3 bg-muted/20 rounded-xl border border-border/40">
                                        <div className="space-y-0.5">
                                            <div className="flex items-center gap-2">
                                                <Power className={cn("w-4 h-4", livePriceEnabled ? "text-emerald-500" : "text-muted-foreground")} />
                                                <label className="text-sm font-bold uppercase tracking-tight">Live Market Streaming</label>
                                            </div>
                                            <p className="text-[10px] text-muted-foreground">
                                                Toggle socket connection for real-time prices.
                                            </p>
                                        </div>
                                        <Switch
                                            checked={livePriceEnabled}
                                            onCheckedChange={setLivePriceEnabled}
                                        />
                                    </div>
                                </div>
                            ...
                            <div className="px-6 py-4 bg-muted/20 border-t border-border flex justify-end">
                                <Button
                                    size="sm"
                                    onClick={handleSaveAliceBlue}
                                    disabled={savingSection === "Alice Blue"}
                                >
                                    {savingSection === "Alice Blue" ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Save className="w-4 h-4 mr-2" />}
                                    Update Alice Blue
                                </Button>
                            </div>
                        </Card>
